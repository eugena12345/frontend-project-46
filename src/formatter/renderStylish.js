const renderStylish = (file1, file2, space = '') => {
      ...
      if (keys1.includes(key)) {
        if (keys2.includes(key)) {
          if (typeof (file1[key]) === 'object' && typeof (file2[key]) === 'object') {
            return `${space}  ${substring}${renderStylish(file1[key], file2[key], newSpace)}\n`
          }
          if (file1[key] === file2[key]) {
            return `${space}  ${substring}${file1[key]}\n`;
          }
          if (typeof (file1[key]) === 'object' && file1[key] !== null) {
            keyForPaint1 = renderStylish(file1[key], file1[key], newSpace);
          } else {
            keyForPaint1 = file1[key]
          }
          if (typeof (file2[key]) === 'object' && file2[key] !== null) {
            console.log(`file2[key]${file2[key]}`);
            keyForPaint2 = renderStylish(file2[key], file2[key], newSpace);
          } else {
